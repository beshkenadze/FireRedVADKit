...
/// Cepstral Mean and Variance Normalization with hardcoded global stats.
///
/// Applies: normalized = (feature - mean) * invStd
/// Stats extracted from FireRedVAD pretrained/Stream-VAD/cmvn.ark
public struct CMVN {
    private init() {}

    /// Global mean (80-dim), extracted from cmvn.ark
    public static let means: [Float] = [
    ...
    /// Global inverse std (80-dim), extracted from cmvn.ark
    /// This is 1/std, so normalized = (x - mean) * invStd
    public static let invStd: [Float] = [
    ...
    /// Apply CMVN normalization in-place using vectorized operations.
    ///
    /// - Parameters:
    ///   - features: Flat [numFrames * 80] fbank features, modified in-place
    ///   - numFrames: Number of frames
    public static func apply(features: inout [Float], numFrames: Int) {
        let dim = 80
        precondition(features.count == numFrames * dim)

        features.withUnsafeMutableBufferPointer { ptr in
            guard let base = ptr.baseAddress else { return }
            for frame in 0 ..< numFrames {
                let offset = frame * dim
                means.withUnsafeBufferPointer { meansPtr in
                    invStd.withUnsafeBufferPointer { invStdPtr in
                        guard let meansBase = meansPtr.baseAddress,
                              let invStdBase = invStdPtr.baseAddress else { return }
                        // x = x - mean
                        vDSP_vsub(meansBase, 1, base + offset, 1, base + offset, 1, vDSP_Length(dim))
                        // x = x * invStd
                        vDSP_vmul(base + offset, 1, invStdBase, 1, base + offset, 1, vDSP_Length(dim))
                    }
                }
            }
        }
